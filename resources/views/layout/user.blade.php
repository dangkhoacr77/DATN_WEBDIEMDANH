...
<body style="background: #f8f9fa; font-family: 'Segoe UI', sans-serif;">
    <nav class="navbar navbar-light bg-primary text-white px-3 d-flex justify-content-between">
      ...
      <div class="d-flex align-items-center gap-3 avatar-menu" onclick="toggleMenu()">
        ...
        @endguest
        <div id="avatarDropdown" class="position-absolute" style="right:0; top:50px; display:none; background:#fff; border:1px solid #ccc; border-radius:5px; min-width:120px; z-index:100;">
          @guest
            <a href="{{ route('xacthuc.dang-nhap') }}" class="d-block" style="padding:10px 15px; color:#000; text-decoration:none;">Đăng nhập</a>
            <a href="{{ route('xacthuc.dang-ky') }}" class="d-block" style="padding:10px 15px; color:#000; text-decoration:none;">Đăng ký</a>
          @else
            <a href="#" class="d-block" style="padding:10px 15px; color:#000; text-decoration:none;">Cài đặt</a>
            <form method="POST" action="#" class="m-0">
              @csrf
              <button type="submit" style="display:block; width:100%; padding:10px 15px; background:none; border:none; text-align:left;">Đăng xuất</button>
            </form>
          @endguest
        </div>
      </div>
                  ...
                  <nav class="nav flex-column">
                    <a href="{{ route('nguoidung.tt-canhan') }}"
                       style="display:block; padding:12px 16px; text-decoration:none;
                              ...
                              background-color: {{ (request()->routeIs('nguoidung.tt-canhan') || Request::is('nguoidung/tt-canhan*')) ? '#e9f1ff' : 'transparent' }};">Thông tin cá nhân</a>
                    <a href="{{ route('nguoidung.ql-bieumau') }}"
                       style="display:block; padding:12px 16px; text-decoration:none;
                              ...
                              background-color: {{ (request()->routeIs('nguoidung.ql-bieumau') || Request::is('nguoidung/ql-bieumau*')) ? '#e9f1ff' : 'transparent' }};">Danh sách biểu mẫu</a>
                    <a href="{{ route('nguoidung.ql-danhsach') }}"
                       style="display:block; padding:12px 16px; text-decoration:none;
                              ...
                              background-color: {{ (request()->routeIs('nguoidung.ql-danhsach') || Request::is('nguoidung/ql-danhsach*')) ? '#e9f1ff' : 'transparent' }};">Danh sách điểm danh</a>
                    <a href="{{ route('nguoidung.ls-diemdanh') }}"
                       style="display:block; padding:12px 16px; text-decoration:none;
                              ...
                              background-color: {{ (request()->routeIs('nguoidung.ls-diemdanh') || Request::is('nguoidung/ls-diemdanh*')) ? '#e9f1ff' : 'transparent' }};">Lịch sử điểm danh</a>
                  </nav>
                </div>
            </div>
    ...
    <!-- Footer -->
    <footer style="background:#1c1f3c; color:#fff;" class="text-center py-5 mt-5">
      <div class="d-flex flex-column align-items-center">
        <div class="d-flex align-items-center mb-2">
          <div style="background:#2dc5c5; color:#fff; width:40px; height:40px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:bold;">LOGO</div>
          <span>Dự án lập trình web</span>
        </div>
        <p>Bạn có hài lòng khi sử dụng trang web?</p>
        <form class="d-flex justify-content-center gap-2 mb-3">
          <input type="text" class="form-control w-200" placeholder="Đánh giá">
          <button type="submit" class="btn" style="background:#17a2b8; color:#fff;">Gửi</button>
        </form>
