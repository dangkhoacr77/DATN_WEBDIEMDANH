...
<body style="{{ $bodyStyle }}" class="min-h-screen bg-cover bg-center bg-no-repeat font-['Inter'] text-gray-800">
    <div class="min-h-screen flex flex-col">
        ...
        <header
            ...
            <button class="hover:bg-white/10 p-2 rounded-full transition duration-300">
        ...
        <main class="flex-1 p-6 overflow-auto">
            <div class="max-w-3xl mx-auto space-y-6">
                <!-- Tiêu đề biểu mẫu -->
                <div
                    class="bg-white p-6 rounded-2xl shadow-md border border-gray-100 hover:shadow-lg transition duration-300 max-w-3xl mx-auto">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 mt-2 text-sm">{{ $bieuMau->mo_ta_tieu_de ?? 'Không có mô tả' }}</p>
                        </div>
                    </div>
                </div>

                <!-- Thông báo lỗi căn giữa riêng -->
                @if (isset($errorMessage))
                    <div class="flex justify-center mt-4">
                        <div
                            class="bg-red-50 border border-red-200 text-red-800 p-4 rounded-xl shadow-lg animate-fade-in-down max-w-fit mx-auto">
                            <div class="flex items-start gap-3">
    ...
    @if ((!isset($hideQuestions) || !$hideQuestions) && !isset($errorMessage))
        <form method="POST" action="{{ route('traloi-bieumau.store') }}" class="space-y-4 animate-fade-in"
            id="form-multi-step">
            @csrf
